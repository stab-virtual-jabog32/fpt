...
const update_loadout_data = function() { 
    ...
    $('select[id^="loadout_"]').each(function() {
        ...
        if (weight !== undefined) payload_weight += weight;
        $('*[data-ref="' + this.id + '"]').text(weight);

        let fuel = $("option:selected", this).data('fuel');
        if (fuel !== undefined) external_fuel += fuel;
    });
...
const update_defaults = function() {
    ...
    $.get(url + '?' + $.param({ t: theater, cs: callsign, n: callsign_number }), function(data) {
        if ('freq' in data) $('#flight_frequency').val(data['freq']);
        if ('group' in data) $('#flight_group_id').val(data['group']);
        if ('laser' in data) $('#flight_laser').val(data['laser']);
        if ('tcn' in data) $('#flight_tacan_channel').val(data['tcn']);
        if ('iff' in data) $('#flight_iff').val(data['iff']);
...
$(document).ready(function() {
    ...
    $('#waypointform').submit(function(event) {
        $.post(window.location.href + '/waypoints/' + $('#waypoint_id').val(), {
            ...
            $('#waypoint_id, #waypoint_insert, #waypoint_name, #waypoint_dme, #waypoint_pos, #waypoint_lat, #waypoint_lon, #waypoint_tot').val('');
